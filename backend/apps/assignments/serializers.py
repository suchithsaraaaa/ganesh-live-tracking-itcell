...
class CreateAssignmentSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        # Station and Zone match between Idol and Constable using canonical normalization
        from common.zones import are_same_ps, are_same_zone
        from common.permissions import check_user_jurisdiction_over_idol

        if not constable.police_station or not are_same_ps(constable.police_station, idol.police_station):
            raise serializers.ValidationError({
        ...
        if not constable.zone or not are_same_zone(constable.zone, idol.zone):
            raise serializers.ValidationError({
                'constable_id': f"Officer's zone ('{constable.zone or 'None'}') does not match idol's zone ('{idol.zone}')."
            })

        # Rule 30: Caller jurisdiction verification using authoritative helper
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            caller = request.user
            allowed, err_msg = check_user_jurisdiction_over_idol(caller, idol)
            if not allowed:
                raise serializers.ValidationError({'error': err_msg or 'Not permitted to this jurisdiction.'})

            # Check caller jurisdiction over the constable being assigned
            caller_zone = (caller.zone or '').strip()
            caller_ps = (caller.police_station or '').strip()
            if caller.role == UserRole.SHO and caller_ps:
                if not are_same_ps(caller_ps, constable.police_station):
                    raise serializers.ValidationError({'constable_id': 'Cannot assign officer from outside your police station.'})
            elif caller.role in [UserRole.SYS_ADMIN, UserRole.ACP] and caller_zone:
                if not are_same_zone(caller_zone, constable.zone):
