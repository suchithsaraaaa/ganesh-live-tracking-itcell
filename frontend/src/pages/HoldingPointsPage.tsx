...
export const HoldingPointsPage: React.FC = () => {
  const { idols, loading, error, reload } = useIdolsByState('HOLDING');
  const { handleSelectIdol } = useTracking();

  const byStation = useMemo(() => {
    const map = new Map<string, typeof idols>();
    idols.forEach((idol) => {
      const key = idol.police_station || 'Unassigned';
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden animate-fade-in-up">
      <div className="px-6 pt-6 pb-4">
        <h1 className="text-lg font-semibold text-text-primary">Holding Points</h1>
        <p className="text-xs text-text-tertiary mt-0.5">
          GPIDs currently in a holding / stoppage state, grouped by police station jurisdiction.
        </p>
      </div>

      <div className="flex-1 overflow-y-auto px-6 pb-6">
        {loading && <LoadingState label="Loading holding-state GPIDs…" />}
        {!loading && error && <ErrorState message={error} onRetry={reload} />}
        {!loading && !error && idols.length === 0 && (
          <EmptyState title="No GPIDs are currently in holding" />
        )}
        {!loading && !error && byStation.length > 0 && (
          <div className="space-y-4">
            {byStation.map(([station, group]) => (
              <div key={station} className="bg-elevated border border-border-subtle rounded-lg overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border-subtle bg-elevated-2/40">
                  <PauseCircle className="w-4 h-4 text-status-warning" strokeWidth={1.8} />
                  <span className="text-sm font-medium text-text-primary">{station}</span>
                ...
                {group.map((idol, i) => (
                  ...
                  >
                    ...
                    <span className="text-text-tertiary">{idol.zone}</span>
                  </button>
                ))}
              </div>
            ))}
